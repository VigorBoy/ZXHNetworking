...
+ (NSString *)ZXHnetworking_md5:(NSString *)string;
...
static ZXHResponseType sg_responseType = ZXHBResponseTypeJSON;
static ZXHRequestType  sg_requestType  = ZXHBRequestTypeJSON;
static ZXHNetworkStatus sg_networkStatus = ZXHBNetworkStatusUnknown;
...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/ZXHNetworkingCaches"];
}
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"ZXHNetworking clear caches error: %@", error);
        } else {
            NSLog(@"ZXHNetworking clear caches ok");
        }
    }
}
...
+ (void)cancelAllRequest {
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(ZXHURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[ZXHURLSessionTask class]]) {
                [task cancel];
            }
        }];
...
+ (ZXHURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(ZXHDownloadProgress)progress
                               success:(ZXHResponseSuccess)success
                                  fail:(ZXHResponseFail)fail {
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            ZXHAppLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    } else {
    ...
    ZXHURLSessionTask *session = nil;
    
    if (httpMethod == 1) {
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == ZXHBNetworkStatusNotReachable ||  sg_networkStatus == ZXHBNetworkStatusUnknown ) {
                    id response = [ZXHNetworking cahceResponseWithURL:absolute
                                                           parameters:params];
                    if (response) {
                        if (success) {
...
+ (ZXHURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        ZXHAppLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        ZXHAppLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (ZXHURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    AFHTTPSessionManager *manager = [self manager];
    ZXHURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (ZXHURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                ZXHAppLog(@"Download success for url %@",
            ...
            if ([self isDebug]) {
                ZXHAppLog(@"Download fail for url %@, reason : %@",
                          [self absoluteUrlWithPath:url],
                          [error description]);
            }
        }
...
+ (AFHTTPSessionManager *)manager {
    ...
    switch (sg_requestType) {
        case ZXHBRequestTypeJSON: {
            manager.requestSerializer = [AFJSONRequestSerializer serializer];
            break;
        }
        case ZXHBRequestTypePlainText: {
    ...
    switch (sg_responseType) {
        case ZXHBResponseTypeJSON: {
            manager.responseSerializer = [AFJSONResponseSerializer serializer];
            break;
        }
        case ZXHBResponseTypeXML: {
            manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
            break;
        }
        case ZXHBResponseTypeData: {
...
+ (void)detectNetwork{
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable){
            sg_networkStatus = ZXHBNetworkStatusNotReachable;
        }else if (status == AFNetworkReachabilityStatusUnknown){
            sg_networkStatus = ZXHBNetworkStatusUnknown;
        }else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
            sg_networkStatus = ZXHBNetworkStatusReachableViaWWAN;
        }else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
            sg_networkStatus = ZXHBNetworkStatusReachableViaWiFi;
        }
    }];
}
...
+ (void)successResponse:(id)responseData callback:(ZXHResponseSuccess)success {
    if (success) {
        success([self tryToParseData:responseData]);
    }
}

+ (NSString *)ZXH_URLEncode:(NSString *)url {
    NSString *newString =
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        NSString *key = [NSString ZXHnetworking_md5:absoluteURL];
        NSString *path = [directoryPath stringByAppendingPathComponent:key];
        
        NSData *data = [[NSFileManager defaultManager] contentsAtPath:path];
        if (data) {
            cacheData = data;
            ZXHAppLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:directoryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error];
            if (error) {
                ZXHAppLog(@"create cache dir error: %@\n", error);
                return;
            }
        }
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                ZXHAppLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                ZXHAppLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
...
+ (void)handleCallbackWithError:(NSError *)error fail:(ZXHResponseFail)fail {
    if ([error code] == NSURLErrorCancelled) {
